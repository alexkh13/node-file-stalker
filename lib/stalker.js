...
class Stalker extends EventEmitter {
    ...
    constructor(options) {
        ...
        if (!fs.existsSync(options.listPath)) {
            return;
        }

        if (fs.lstatSync(listPath).isDirectory()) {
            this._options.listPath = path.resolve(listPath, 'index.txt');
        }
        ...
        return new Promise((resolve) => {
            ...
            lineReader.on('line', (line) => {
                ...
                if (source.type === 'FILE') {
                    ...
                    pProcess.push(this._process(source));
                }
    ...
    async _process(source) {
        ...
        if (newHash) {
            ...
            await this._redisCall('hset', key, 'hash', newHash);
        }
        ...
        switch(source.type) {
            case 'FILE':
                if (!fs.existsSync(source.filePath)) {
                    this.emit('fail', "file not found '" + source.filePath + "'");
                }
                else {
    ...
    async _getAdded(source, keys) {
        ...
        let hkey = this._key(source, 'keys');
        let values = keys.length ? await this._redisCall.apply(this, ['hmget', hkey].concat(keys)) : [];
